...
#define DELAY_INIT  500   /* 初始延时 500ms = 2Hz，方便仿真观察 */
#define DELAY_STEP  100   /* 每次按键减少 100ms */
...
*        KEY1/KEY2 按下后至少亮半个周期才生效，防止LED脉冲
*        LED之间无缝切换，一个灯灭下一个灯立刻亮
* @param argument: Not used
* @retval None
*/
...
void StartLEDTask(void *argument)
{
  ...
  /* 上电先点亮第一个LED */
  HAL_GPIO_WritePin(leds[g_direction == 1 ? step : (2 - step)].port,
                    leds[g_direction == 1 ? step : (2 - step)].pin, GPIO_PIN_SET);
  ...
  for(;;)
  {
    ...
    if (!(flags > 0x80000000U))  /* 收到有效事件 */
    {
      ...
      if (flags & EVT_KEY1)
        pend_dir = 1;                  /* 方向：标记待生效 */
      if (flags & EVT_KEY2)
        pend_speed = 1;                /* 速率：标记待生效 */
    }

    /* ---- 亮完前半周期 ---- */
    half_ms = g_delay_ms / 2;
